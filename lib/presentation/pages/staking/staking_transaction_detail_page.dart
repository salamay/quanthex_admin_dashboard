...
class StakingTransactionDetailPage extends StatelessWidget {
  final StakingPaymentModel payment;

  const StakingTransactionDetailPage({super.key, required this.payment});
          ...
          children: [
            ...
            _buildSection(
              ...
              children: [
                MiningInfoRow(
                  label: 'ID',
                  value: _truncateAddress(payment.spId),
                ),
                const SizedBox(height: 8),
                MiningInfoRow(
                  label: 'Status',
                  value: _capitalizeStatus(payment.spStatus),
                ),
                const SizedBox(height: 8),
                MiningInfoRow(
                  label: 'Plan',
                  value: payment.spStakingPlan,
                ),
                const SizedBox(height: 8),
                MiningInfoRow(
                  label: 'Payment Cycle',
                  value: payment.cycleLabel,
                ),
                const SizedBox(height: 8),
                MiningInfoRow(
                  label: 'Referrals at Payment',
                  value: '${payment.spReferralCountAtPayment}',
                ),
                const SizedBox(height: 8),
                MiningInfoRow(
                  label: 'Chain ID',
                  value: '${payment.spChainId}',
                ),
                const SizedBox(height: 8),
                MiningInfoRow(
                  label: 'Reward Symbol',
                  value: payment.spRewardSymbol ??
                      payment.stakingRewardAssetSymbol ??
                      'N/A',
                ),
              ],
            ),
            ...
            _buildSection(
              ...
              children: [
                MiningInfoRow(
                  label: 'Email',
                  value: payment.spEmail,
                ),
                const SizedBox(height: 8),
                MiningInfoRow(
                  label: 'User ID',
                  value: _truncateAddress(payment.spUid),
                ),
                const SizedBox(height: 8),
                MiningInfoRow(
                  label: 'Staking ID',
                  value: _truncateAddress(payment.spStakingId),
                ),
                const SizedBox(height: 8),
                MiningInfoRow(
                  label: 'Wallet Address',
                  value: _truncateAddress(payment.stakingWalletAddress),
                ),
              ],
            ),
            ...
            _buildSection(
              ...
              children: [
                ...
                MiningInfoRow(
                  label: 'Staked Asset',
                  value: payment.stakedAssetSymbol ?? 'N/A',
                ),
                const SizedBox(height: 8),
                MiningInfoRow(
                  label: 'Reward Asset',
                  value: payment.stakingRewardAssetName ?? 'N/A',
                ),
                const SizedBox(height: 8),
                MiningInfoRow(
                  label: 'Reward Chain',
                  value: payment.stakingRewardChainName ?? 'N/A',
                ),
              ],
            ),
            ...
            _buildSection(
              ...
              children: [
                if (payment.isConfirmed &&
                    payment.spTxHash != null &&
                    payment.spTxHash!.isNotEmpty) ...[
                  MiningInfoRow(
                    label: 'TX Hash',
                    value: _truncateAddress(payment.spTxHash),
                  ),
                  const SizedBox(height: 8),
                ],
                MiningInfoRow(
                  label: 'Created',
                  value: _formatDate(payment.spCreatedAt),
                ),
                const SizedBox(height: 8),
                MiningInfoRow(
                  label: 'Updated',
                  value: _formatDate(payment.spUpdatedAt),
                ),
              ],
            ),
